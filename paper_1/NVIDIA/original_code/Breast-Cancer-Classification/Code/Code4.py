...
criterion = nn.BCEWithLogitsLoss(pos_weight=torch.tensor(class_weights[1]).to(device))
...
for epoch in range(num_epochs):
    ...
    for phase in ['train', 'val']:
        if phase == 'train':
            ...
            dataloader = DataLoader(train_dataset, batch_size=32, sampler=weighted_sampler)
        else:
        ...
        if phase == 'train':
            train_acc_history.append(epoch_acc.to(device).numpy())
            train_loss_history.append(epoch_loss)
        else:
            val_acc_history.append(epoch_acc.to(device).numpy())
...
plt.savefig('/home/data3/Ali/Code/Saina/Brea/OutPut/figure.png')
